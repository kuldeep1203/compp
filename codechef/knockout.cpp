...
void fun(){
    vector<int>contestants(16);
    vector<int>x(16);
    unordered_map<int,int>winners;
    for(int i=0;i<16;i++){
        cin>>contestants[i];
        winners[contestants[i]]=0;
    }
    x=contestants;
    sort(contestants.begin(),contestants.end());
    winners[contestants[15]]=4;
    winners[contestants[0]]=0;
    for(int i=1;i<=14;i++){
        if(i<=2) winners[contestants[i]]=1;
        else if(i<=6) winners[contestants[i]]=2;
        else if(i<=14) winners[contestants[i]]=3;
    }
    for(int i=0;i<16;i++){
        cout<<winners[x[i]]<<" ";
    }
